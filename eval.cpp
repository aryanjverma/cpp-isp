...
int main(){    
    ...
    for(int i = 100; i < 110; i++){
        for(int j = 0; j < 10; j++){
            string path = "dataset/" + to_string(j) + "/" + to_string(j) + "/" + to_string(i) + ".png";
            vector<float> image = process_image(path);
            vector<float> prediction = nn.feed_forward(image);
            int max_index = max_element(prediction.begin(), prediction.end()) - prediction.begin();
            if(max_index == j){
                accuracy += 1;
            }
            cout << "Prediction: " << max_index << "\n";
            cout << "Expected: " << j << "\n";
        }
    }
    accuracy /= 100;
